        ...
        {mode === 'surgery' ? (
          <div className="flex-1 overflow-y-auto bg-slate-50/50 p-4 sm:p-6 custom-scrollbar space-y-4">
            ...
            <div className="bg-white rounded-xl border border-slate-200/60 p-3.5 sm:p-4 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2 bg-emerald-50/70 border border-emerald-100/80 px-3.5 py-2 rounded-xl shrink-0 self-start sm:self-auto">
                ...
                <div className="flex items-baseline gap-1">
                  ...
                  <span className="text-[10px] font-bold text-emerald-600">YER</span>
                </div>
                {discountAmount > 0 && (
                  <span className="text-[10px] font-bold text-amber-600 pr-1.5 border-r border-emerald-200/70">
                    (خصم {discountAmount.toLocaleString('en-US')} YER)
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200/60 p-4 sm:p-5 shadow-xs space-y-4">
              <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-bold text-slate-700 text-sm">بيانات العملية المالية</h3>
                </div>
                {remainingAmount <= 0 && (
                ...
                <div className="space-y-4 pt-1">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 sm:p-3.5 rounded-xl bg-slate-50/70 border border-slate-200/60">
                    <div className="flex flex-wrap items-center gap-2.5">
                      ...
                      {/* تصميم مطابق تماماً لحقل سعر الوحدة في المخزن (مدمج، أزرار بالأطراف، YER بجوار النص، أرقام إنجليزية) */}
                      <div className="relative flex items-center h-[38px] w-48 sm:w-52 bg-white border border-slate-200 rounded-xl overflow-hidden focus-within:border-emerald-500 focus-within:ring-1 focus-within:ring-emerald-500 transition-all shadow-2xs group shrink-0">
                        ...
                        {/* حقل الإدخال مع اختصار YER قريب جداً من النص في المنتصف */}
                      ...
                      <button
                        ...
                        className="px-2.5 py-1.5 text-[11px] font-bold text-teal-700 bg-teal-50 hover:bg-teal-100 border border-teal-200/80 rounded-lg transition-all active:scale-95 cursor-pointer flex items-center gap-1 shrink-0 shadow-2xs"
                    ...
                    {/* عرض المبلغ نصاً بنفس الصف */}
                    {parseFloat(surgeryInstallment || 0) > 0 ? (
                      <div className="flex items-center gap-1.5 text-xs font-bold text-emerald-700 bg-emerald-50/80 border border-emerald-100/80 px-3 py-1.5 rounded-lg shrink-0 self-start sm:self-auto">
                        <span className="text-[10px] text-emerald-600 font-medium">كتابةً:</span>
                        <span>{taffyot(parseFloat(surgeryInstallment)).replace(/(ريالاً يمنياً|ريال يمني|فقط|لا غير|-)/g, '').trim()} ريال يمني.</span>
                      </div>
                    ) : (
                      <div className="text-[11px] font-medium text-slate-400 self-start sm:self-auto">
                      ...
                      >
                        {isSubmitting ? (
                          ...
                          <>
                            ...
                            <span>
                              {parseFloat(surgeryInstallment || 0) > 0
                                ? `تأكيد السداد (${parseFloat(surgeryInstallment).toLocaleString('en-US')} YER)`
                                : 'تأكيد السداد'}
                            </span>
                          </>
                        )}
