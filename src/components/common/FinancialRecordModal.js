...
import { Printer, Maximize2, Minimize2, X, History, ArrowUpRight, ArrowDownLeft, FileText, User, Phone, Hash, Layers, Calendar, Scissors, Clock, CheckCircle2 } from 'lucide-react';
                            ...
                            <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                              ...
                              <td className="px-5 py-3 align-middle">
                                <div className="flex items-center gap-1 text-slate-800">
                                  ...
                                  <span className="text-[10px] font-bold text-slate-400">ر.ي</span>
                                </div>
                              </td>
                    ...
                    return (
                      <div className={`flex items-center justify-between px-6 py-4 border-t mt-auto print:bg-transparent print:border-slate-800 print:border-t-2 ${bgClass}`}>
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className={`text-[10px] font-bold ${currencyClass}`}>ر.ي</span>
                      ...
                      <table className="w-full text-right text-sm whitespace-nowrap">
                        <thead className="bg-slate-50 border-b border-slate-200">
                          <tr>
                            ...
                            <th className="px-5 py-3.5 text-xs font-bold text-slate-500 w-48">الحالة</th>
                            <th className="px-5 py-3.5 text-xs font-bold text-slate-500 w-48">تاريخ الطلب</th>
                            <th className="px-5 py-3.5 text-xs font-bold text-slate-500 w-36">المبلغ</th>
                          ...
                          {filteredHistory.map((s, index) => {
                            ...
                            <tr key={'hist_'+index} className="hover:bg-slate-50/50 transition-colors">
                              ...
                              <td className="px-5 py-4">
                                <span className={`inline-flex items-center gap-1.5 text-[11px] font-bold ${badge.color.split(' ').find(c => c.startsWith('text-')) || 'text-slate-600'}`}>
                              ...
                              <td className="px-5 py-4">
                                <div className="flex items-baseline gap-1 text-base font-black tracking-tight text-slate-800">
                                  <span dir="ltr">{parseFloat(s.final_price || s.price).toLocaleString('en-US')}</span>
                              ...
                              <td className="px-5 py-4">
                                {s.status === 'paid' ? (
                                  ...
                                  <span className="text-slate-400 font-bold text-[9px] sm:text-[10px] block whitespace-nowrap">
                                    لا يمكن الاسترداد ({s.status === 'in_progress' ? 'قيد التنفيذ' : 'مكتمل'})
                                  </span>
                                ) : null}
                              </td>
                            </tr>
                ...
                <div className="space-y-3 mt-8">
                  ...
                  <div className="flex flex-col gap-4">
                    {recordData.surgeries.map(surgery => {
                      ...
                      const discountAmount = parseFloat(surgery.discount_amount || 0);
                      
                      return (
                        <div key={surgery.surgery_id} className="bg-white rounded-xl border border-slate-200/60 p-3.5 sm:p-4 shadow-xs flex flex-col gap-4">
                          {/* شريط بيانات العملية ورسومها */}
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            <div className="flex items-center gap-3">
                              ...
                              <h3 className="font-bold text-slate-800 text-sm sm:text-base">
                                {surgery.surgery_type || 'عملية جراحية'}
                              </h3>
                            </div>

                            {/* رسوم العملية وزر السداد */}
                            {/* رسوم العملية */}
                            <div className="flex items-center gap-2 flex-wrap bg-emerald-50/70 border border-emerald-100/80 px-3.5 py-2 rounded-xl shrink-0">
                              ...
                              {netPrice > 0 && (
                                <span className="text-[11px] font-bold text-emerald-700/90 pr-2 border-r border-emerald-200/70 hidden sm:inline-block">
                          ...
                          {/* حالة الدفع (الإحصائيات) */}
                          {paid > 0 && remaining > 0 ? (
                            <div className="bg-white rounded-xl border border-slate-200/60 shadow-xs overflow-hidden mt-1">
                              <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x sm:divide-x-reverse divide-slate-100">
                                ...
                                <div className="p-3.5 sm:p-4 flex flex-col justify-between bg-rose-50/20">
                                  ...
                                  <span className="text-[10px] font-bold text-rose-600 mt-1 block">
                                    متبقي سداده ({100 - Math.round(progressPct)}%)
                                  </span>
                                </div>
                              </div>
                            ...
                            <div className="p-3.5 sm:p-4 rounded-xl bg-emerald-50 border border-emerald-200 flex flex-col sm:flex-row items-center justify-center gap-2.5">
                              <CheckCircle2 size={20} className="text-emerald-600" />
                              <span className="font-bold text-emerald-800 text-sm">تم تسديد رسوم العملية بالكامل.</span>
                            </div>
                          ) : (
                            <div className="p-3.5 sm:p-4 rounded-xl bg-slate-50 border border-slate-200 flex flex-col sm:flex-row items-center justify-center gap-2.5">
                              <Clock size={20} className="text-slate-500" />
                              <span className="font-bold text-slate-700 text-sm">بانتظار تسديد الرسوم</span>
                            </div>
                          )}
                          ...
                          {remaining > 0 && (
                            <div className="flex justify-end mt-1">
                              <button
                                onClick={() => {
                                  if (typeof onOpenCheckout === 'function') {
                                    const surgeryVisit = recordData?.visits?.find(v => v.id === surgery.visit_id);
                                    onOpenCheckout({ 
                                      ...surgery, 
                                      id: surgery.surgery_id, 
                                      is_surgery: true,
                                      visit_number: surgeryVisit?.visit_number 
                                    });
                                  }
                                }}
                                className="bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-black px-6 py-2.5 rounded-xl shadow-sm hover:shadow-md transition-all active:scale-95 shrink-0 whitespace-nowrap flex items-center justify-center"
                              >
                                تسديد الرسوم
                              </button>
                            </div>
        ...
        <div className="p-6 space-y-5">
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
            ...
            <div className="flex items-center justify-between mt-2">
              ...
              <span className="text-sm font-black text-slate-700" dir="ltr">{parseFloat(refundTarget.final_price || refundTarget.price).toLocaleString('en-US')} <span className="text-[10px]">ر.ي</span></span>
            </div>
          </div>
          <div>
