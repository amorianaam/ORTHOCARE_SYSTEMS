...
const Dashboard = () => {
  ...
  const fetchTransactions = useCallback(async () => {
    setTxLoading(true);
    try {
      let queryUrl = `/api/inventory/general/dashboard/transactions?page=${txPage}&limit=10&dateFilter=${txDateFilter}&typeFilter=${txTypeFilter}&grouped=false`;
  ...
  const handleExportTransactions = async () => {
    try {
      let queryUrl = `/api/inventory/general/dashboard/transactions?dateFilter=${txDateFilter}&typeFilter=${txTypeFilter}&export=true&grouped=false`;
      if (txDateFilter === 'custom' && txStartDate && txEndDate) {
        queryUrl += `&startDate=${txStartDate}&endDate=${txEndDate}`;
      }
      const res = await fetch(queryUrl, {
    ...
    <div className="p-6 min-h-full flex flex-col space-y-6" dir="rtl">
      ...
      <div className="flex items-center justify-between mb-2 flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-gray-800">لوحة تحكم المخزن العام</h1>
