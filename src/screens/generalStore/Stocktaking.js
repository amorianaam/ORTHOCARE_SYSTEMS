...
const StaleDraftAlert = ({ staleItems, sessionId, token, onRefreshed }) => {
  ...
  const handleRefresh = async () => {
    setRefreshing(true);
    try {
      const res = await fetch(`/api/inventory/general/stocktaking/${sessionId}/refresh-expected`, {
        method: 'PATCH',
...
const SessionDetail = ({ sessionId, token, onBack, onFinalize, onEdit, onDelete }) => {
  ...
  const loadSession = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/inventory/general/stocktaking/${sessionId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const checkStaleness = useCallback(async () => {
    try {
      const res = await fetch(`/api/inventory/general/stocktaking/${sessionId}/staleness-check`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const d = await res.json();
      if (res.ok) {
  ...
  const handleFinalize = async () => {
    setFinalizing(true);
    try {
      const res = await fetch(`/api/inventory/general/stocktaking/${sessionId}/finalize`, {
        method: 'PATCH',
        headers: { Authorization: `Bearer ${token}` }
      });
      const d = await res.json();
...
const Stocktaking = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [sessRes, itemsRes] = await Promise.all([
        fetch('/api/inventory/general/stocktaking', { headers: { Authorization: `Bearer ${token}` } }),
  ...
  const checkStaleness = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/inventory/general/stocktaking/${activeSessionId}/staleness-check`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const d = await res.json();
      if (res.ok) {
  ...
  const handleRefreshed = async () => {
    if (!activeSessionId) return;
    try {
      const res = await fetch(`/api/inventory/general/stocktaking/${activeSessionId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const d = await res.json();
      if (res.ok) {
  ...
  const submitStocktaking = async (isDraft) => {
    ...
    try {
      ...
      const endpoint = isUpdate ? `/api/inventory/general/stocktaking/${activeSessionId}` : '/api/inventory/general/stocktaking';
      ...
      if (isUpdate && !isDraft) {
        const finRes = await fetch(`/api/inventory/general/stocktaking/${activeSessionId}/finalize`, {
          method: 'PATCH',
          headers: { Authorization: `Bearer ${token}` }
        });
