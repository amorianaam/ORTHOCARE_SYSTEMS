...
import { PatientDetailPanel } from "../../components/radiology/RadiologyModals";

// ── Constants ─────────────
const TABS = [
  { id: 'pending', label: 'الطلبات الواردة', icon: Clock, color: 'text-sky-600', bg: 'bg-sky-50', activeBg: 'bg-sky-600' },
  { id: 'in_progress', label: 'قيد الإجراء', icon: Radiation, color: 'text-violet-600', bg: 'bg-violet-50', activeBg: 'bg-violet-600' },
  { id: 'completed', label: 'فحوصات مكتملة', icon: CheckSquare, color: 'text-emerald-600', bg: 'bg-emerald-50', activeBg: 'bg-emerald-600' },
];
...
const RadiologyDashboard = () => {
  ...
  const fetchStats = useCallback(async (signal) => {
    try {
      ...
      const url = `/api/radiology/stats?startDate=${today}&endDate=${today}&quickStats=1`;
      const res = await fetch(url, { headers, signal });
      if (res.ok) {
        const data = await res.json();
        setDailyStats({
          largeFilms: data.summary?.large_films || 0,
          smallFilms: data.summary?.small_films || 0,
          digitalSaves: data.summary?.without_film || 0,
          totalCompleted: data.summary?.total_operations || 0,
        });
        if (data.tabCounters) {
  ...
  useEffect(() => {
    ...
    socket.on("radiology:updated", handleUpdate);
    ...
    return () => {
      ...
      socket.off("radiology:updated", handleUpdate);
    };
  ...
  const emptyState = useMemo(() => {
    if (query) return { icon: Radiation, title: 'لا توجد سجلات مطابقة', desc: 'لم يتم العثور على مرضى بهذا الاسم أو الرقم المرجعي.' };
    switch (activeTab) {
      case 'pending': return { icon: Clock, title: 'لا توجد طلبات واردة', desc: 'جميع الطلبات تم التعامل معها ولا يوجد مرضى في الانتظار.' };
      case 'in_progress': return { icon: Radiation, title: 'لا يوجد فحوصات قيد الإجراء', desc: 'لم يتم تحويل أي مرضى لغرفة التصوير حالياً.' };
      case 'completed': return { icon: CheckSquare, title: 'لا يوجد فحوصات منجزة', desc: 'لم يتم اعتماد أي فحوصات لهذا اليوم بعد.' };
      default: return { icon: Radiation, title: 'لا توجد سجلات', desc: 'القائمة فارغة.' };
    }
  }, [activeTab, query]);

    const statsCards = useMemo(() => [
    { label: 'أفلام كبيرة مستخدمة', value: dailyStats.largeFilms, icon: Film, color: 'text-indigo-600', bg: 'bg-indigo-50', border: 'border-indigo-100' },
    { label: 'أفلام صغيرة مستخدمة', value: dailyStats.smallFilms, icon: Film, color: 'text-cyan-600', bg: 'bg-cyan-50', border: 'border-cyan-100' },
    { label: 'حفظ رقمي بدون فيلم', value: dailyStats.digitalSaves, icon: ImageIcon, color: 'text-violet-600', bg: 'bg-violet-50', border: 'border-violet-100' },
    { label: 'إجمالي الأشعات المكتملة', value: dailyStats.totalCompleted, icon: CheckCircle, color: 'text-emerald-600', bg: 'bg-emerald-50', border: 'border-emerald-100' },
  ], [dailyStats]);
    ...
    <div className="p-6 min-h-full flex flex-col space-y-6" dir="rtl">
      ...
      <div className="flex items-center justify-between mb-2 flex-shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-blue-700 text-white flex items-center justify-center shadow-lg">
            <Radiation size={20} />
          </div>
          <div>
        ...
        <>
          <div className="flex flex-col md:flex-row items-center gap-4 mb-4">
              ...
              <button
                ...
                className="h-10 w-10 flex flex-shrink-0 items-center justify-center p-0 rounded-xl bg-blue-50 text-blue-600 hover:bg-blue-700 hover:text-white transition-all border border-blue-100 shadow-sm"
              >
                  ...
                  return (
                    <div
                      ...
                      className={`rounded-2xl border p-5 cursor-pointer transition-all ${
                        isActive
                          ? "border-blue-500 shadow-md bg-blue-50/40 ring-2 ring-blue-50 scale-[1.01]"
                      ...
                      <div className="flex justify-between items-center w-full">
                        ...
                        <div className="flex items-center gap-5">
                          <div className="text-left flex flex-col items-end gap-1.5">
                            {activeTab === 'pending' && (
                              <span className="text-xs font-bold bg-sky-50 text-sky-700 px-3 py-1.5 rounded-lg border border-sky-200 flex items-center gap-1.5">
                                <Clock size={14} /> {v.total_tests} أشعة مطلوبة
                              </span>
                            )}
                            ...
                            {activeTab === 'in_progress' && (
                              <span className="text-xs font-bold bg-violet-50 text-violet-700 px-3 py-1.5 rounded-lg border border-violet-200 flex items-center gap-1.5">
                                <Radiation size={14} /> {v.total_tests} أشعة قيد الإجراء
                              </span>
                            )}
                            ...
                            {activeTab === 'completed' && (
                              <span className="text-xs font-bold bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded-lg border border-emerald-200 flex items-center gap-1.5">
                                <CheckSquare size={14} /> {v.total_tests} أشعة منجزة
                              </span>
                            )}
                            {v.tests_with_film > 0 && (
                              <span className="text-[10px] font-bold text-blue-600 flex items-center gap-1 bg-blue-50 px-2 py-1 rounded border border-blue-100">
                          ...
                          {activeTab === "pending" && (
                            <button
                              ...
                              className="text-sm font-bold bg-sky-600 text-white px-5 py-2.5 rounded-xl hover:bg-sky-700 shadow-sm transition-all active:scale-95 flex items-center gap-2">
                              <CheckCircle size={18} /> تحويل لغرفة التصوير
                            </button>
                          )}
                          {activeTab !== "pending" && (
                            <div className="text-blue-400 bg-blue-50 p-2 rounded-xl">
                              <ChevronLeft size={20} />
                            </div>
                          )}
                        </div>
